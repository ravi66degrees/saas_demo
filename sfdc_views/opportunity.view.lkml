view: opportunity {
  ...
  dimension: is_active_contract {
    ...
    and (coalesce(CAST(${cancellation_date} as date),${end_date}) is NULL or coalesce(CAST(${cancellation_date} as date),${end_date}) > current_date() );;
  }
  ...
  dimension: is_active_contract_calendar {
    ...
    sql:  ${stage_name} = 'Closed Won'
        --add buffer window
          ...
          and (coalesce(CAST(${cancellation_date} as date),${end_date}) is NULL or date_add(coalesce(CAST(${cancellation_date} as date),${end_date}), interval 30 day) > ${activity_calendar.activity_date});;
  }
