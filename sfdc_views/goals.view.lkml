view: goals {
  ...
  dimension: year {
    hidden: yes
    type: number
    sql: case when ${TABLE}.Year =2019 then EXTRACT(YEAR FROM current_date())-3
    when ${TABLE}.Year =2020 then EXTRACT(YEAR FROM current_date())-2
    when ${TABLE}.Year =2021 then EXTRACT(YEAR FROM current_date())-1
    when ${TABLE}.Year =2022 then EXTRACT(YEAR FROM current_date()) else ${TABLE}.Year end ;;
  }
