view: account_weather {
  label: "Account Weather ⛅"
  derived_table: {
    sql_trigger_value: select current_date() ;;
    # partition_keys: ["date"]
    # cluster_keys: ["account_id"]
    # requires ID, latitude, longitude columns in accounts table
    sql: WITH weather_raw AS (SELECT id,date,element,value,mflag,qflag,sflag,time FROM `bigquery-public-data.ghcn_d.ghcnd_2020`
        ...
        FROM weather_raw
        GROUP BY date,id),
        distances AS (SELECT account.id as account_id
        ,stations.id AS station_id
        ,ST_DISTANCE(ST_GEOGPOINT(account.billing_longitude,account.billing_latitude),ST_GEOGPOINT(stations.longitude,stations.latitude)) as dist
        FROM ${account.SQL_TABLE_NAME} account
        CROSS JOIN `bigquery-public-data.ghcn_d.ghcnd_stations` stations)
        SELECT distances.account_id
  ...
  dimension: pk {
    ...
    primary_key: yes
    type: string
    sql: CONCAT(CAST(${weather_date} AS STRING),'-',CAST(${account_id} AS STRING)) ;;
  }
